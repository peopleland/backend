package main

import (
	env_config "backend/lib/config"
	"backend/lib/helper"
	"encoding/json"
	"github.com/golang-jwt/jwt"
	"github.com/labstack/echo/v4"
	"github.com/stretchr/testify/assert"
	"net/http"
	"net/http/httptest"
	"os"
	"strings"
	"testing"
)

func Test_changeProfile(t *testing.T) {
	privateKeyPem := "-----BEGIN RSA PRIVATE KEY-----\\n-----END RSA PRIVATE KEY-----\n"
	pulbicKeyPem := "-----BEGIN PUBLIC KEY-----\\nNQIDAQAB\n-----END PUBLIC KEY-----\n"

	os.Setenv("PEOPLELAND_JWT_RSA_PRIVATE_KEY_PEM", privateKeyPem)
	os.Setenv("PEOPLELAND_JWT_RSA_PUBLIC_KEY_PEM", pulbicKeyPem)
	os.Setenv("PEOPLELAND_FAUNADB_SECRET", "fnAEbMoNoqACTLHGcQsRsF5FqEL9YDyoNefP2ozf")

	env_config.BuildConfig()
	env_config.InitFaunadbClient()

	address := "0x3946d96a4b46657ca95CBE85d8a60b822186Ad1f"
	claims := jwt.MapClaims{"address": address}
	exp := int64(86400)
	jwtStr, err := helper.EncodeJwt(claims, privateKeyPem, exp)
	if err != nil {
		t.Failed()
	}
	assert.NotEmpty(t, jwtStr)

	updateData := map[string]string{
		"name": "haha",
	}
	dataBody, err := json.Marshal(updateData)
	if err != nil {
		t.Failed()
	}

	e := echo.New()
	req := httptest.NewRequest(http.MethodGet, "/", strings.NewReader(string(dataBody)))
	req.Header.Set(echo.HeaderAuthorization, jwtStr)
	rec := httptest.NewRecorder()
	c := e.NewContext(req, rec)

	if assert.NoError(t, changeProfile(c)) {
		assert.Equal(t, http.StatusOK, rec.Code)

		var body map[string]interface{}
		err = json.Unmarshal(rec.Body.Bytes(), &body)
		if err != nil {
			t.Failed()
		}
		assert.NotEmpty(t, body["data"])

		data := body["data"].(map[string]interface{})
		assert.Equal(t, data["address"], address)
		assert.Equal(t, data["name"], "haha")
	}
}
