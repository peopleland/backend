...
type PeopleLandContractTheGraphQueryByFromTokenId struct {
	TokenInfos []PeopleLandContractTheGraphTokenInfo `graphql:"tokenInfos(first: 4, where: {tokenId_gte: $tokenId_gte}, orderBy: tokenId)"`
	Meta       PeopleLandContractTheGraphMeta        `graphql:"_meta"`
}

type PeopleLandContractTheGraphQueryByFromTimestamp struct {
	TokenInfos []PeopleLandContractTheGraphTokenInfo `graphql:"tokenInfos(first: 4, where: {givedAtTimestamp_gte: $givedAtTimestamp_gte}, orderBy: tokenId)"`
	Meta       PeopleLandContractTheGraphMeta        `graphql:"_meta"`
}
